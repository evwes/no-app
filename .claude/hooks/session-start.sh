...
echo "  DURABLE SCHEDULER: MCP Routine 'wampo hourly cycle (self-bind)'"
echo "  (trig_017vdX5dSSYh5v68Cwe6EUBu, created 2026-09-08) wakes the main web"
echo "  session hourly at :07. Fresh-session Routines are DEAD: their sessions"
echo "  get no repo source and no push credential — ~40 firings landed nothing"
echo "  (diagnosed 2026-09-08, docs/hourly-cycle-prompt.md). Verify with"
echo "  list_triggers; only if the Routine is GONE, recreate the self-bind from"
echo "  the session that owns the repo per that doc."
