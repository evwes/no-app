...
- **Known residuals, EXACT — corrected 2026-09-08 after the final-year split.**
  ...
  | 527 | 397,007 | $24.1B | `nohead` no heading seeded a region. SPLIT by `ds` (v113): 484 permanently unreachable, 43 ours. The `readfail` half of "ours" is CLOSED by v114 — the statutory column caption now seeds a region, and 28 of those 32 plans publish a real menu; the residue is 2 unlinked-trust pointers (Genentech $14.3B), 1 truncated attachment, 1 CONSOLIDATED filing. What remains ours here is the 11 `nohead/unread` plans ($178M) — OPENED WHOLE 2026-09-10: **6 are ours** ($69M, ~2,600p) with a real fund table under a RECORDKEEPER heading (`STATEMENT OF NET ASSETS` template, `SUMMARY OF NET TRUST ASSETS`, `Plan Investment Vehicle Summary`, `Overview - Summary By Fund`, `Current Plan Assets`); **5 are not** ($109M, ~5,500p — Commercial Vehicle Group's $67M page is a cash-flow statement, Austin 3(16) is a POOLED EMPLOYER roster of per-employer balances, three are Statements of Changes in Net Assets). Fix = add those titles to the v114 retry seed list, gated identically; NOTE `SUMMARY OF NET TRUST ASSETS` is currently in `stopRe` as a region TERMINATOR, the same phrase-does-both-jobs trap as `trusteeHead`. Not worth a version bump alone — bundle it |
